...
function shouldHideFolder(folderPath) {
  ...
  return hiddenConfig.hiddenPaths.some(hiddenPath => {
    ...
    return normalizedFolderPath === normalizedHiddenPath || 
...
function findDocumentByName(docName, includeArchived = false) {
  return documentIndex.find(doc => {
    ...
    return cleanFileName.includes(cleanDocName) || cleanDocName.includes(cleanFileName);
  });
}

// MANDATORY RECORDS FUNCTIONS
const MANDATORY_RECORDS_PATH = path.join(__dirname, 'mandatory-records-index.json');
...
function loadMandatoryRecords() {
  ...
  return {
    "Internal Audit Records": {
      ...
      lastUpdated: null,
      documents: [],
      autoDetectKeywords: ["internal audit", "audit schedule", "audit report", "audit plan", "internal audit checklist"],
      enrichedDocuments: []
    },
    "Management Review Records": {
      ...
      lastUpdated: null,
      documents: [],
      autoDetectKeywords: ["management review", "management meeting", "review minutes", "mgmt review", "management review minutes"],
      enrichedDocuments: []
    },
    "NCR/Corrective Action Records": {
      ...
      lastUpdated: null,
      documents: [],
      autoDetectKeywords: ["ncr", "non-conformance", "corrective action", "non conformance", "corrective action register", "ncr register"],
      enrichedDocuments: []
    },
    "Training/Skills Register": {
      ...
      lastUpdated: null,
      documents: [],
      autoDetectKeywords: ["training register", "skills register", "training matrix", "competency", "training records", "skills matrix"],
      enrichedDocuments: []
    },
    "Risk Management Records": {
      type: "mandatory",
      description: "Company Risk Management/SWOT Analysis or Management Review Improvement plans",
      priority: "high",
      lastUpdated: null,
      documents: [],
      autoDetectKeywords: ["risk management", "swot analysis", "risk register", "improvement plan", "risk assessment", "swot"],
      enrichedDocuments: []
    },
    "Maintenance Contract Projects": {
      type: "mandatory",
      description: "Project records for 2 Maintenance Contracts within the last twelve months",
      priority: "high",
      lastUpdated: null,
      documents: [],
      autoDetectKeywords: ["maintenance contract", "maintenance project", "preventive maintenance", "corrective maintenance", "maintenance schedule"],
      enrichedDocuments: [],
      requiredCount: 2,
      timeframe: "12 months"
    },
    "Construction Projects": {
      type: "mandatory",
      description: "Project records for 2 Construction projects within the last twelve months",
      priority: "high",
      lastUpdated: null,
      documents: [],
      autoDetectKeywords: ["construction project", "building project", "construction contract", "project plan", "construction records"],
      enrichedDocuments: [],
      requiredCount: 2,
      timeframe: "12 months"
    }
...
function buildFileIndex() {
  ...
  try {
      ...
      files.forEach(file => {
        try {
          ...
          if (stat.isDirectory()) {
            ...
            const validExtensions = ['.doc', '.docx', '.pdf', '.xls', '.xlsx', '.ppt', '.pptx', '.txt', '.rtf'];
            ...
            if (validExtensions.includes(ext)) {
              const fileInfo = {
                ...
                isArchived: isArchivedDocument(filePath) // Add archive detection
              };
    ...
    if (fs.existsSync(DOCUMENTS_ROOT)) {
      ...
      buildSearchIndex();
      saveIndexToFile();
      console.log(`Indexing complete. ${documentIndex.length} documents indexed.`);
    } else {
...
function setupWatcher() {
  try {
    if (DOCUMENTS_ROOT.startsWith('\\\\') || DOCUMENTS_ROOT.match(/^[a-zA-Z]:/)) {
    ...
    watcher
      ...
      .on('error', error => {
        ...
        console.log('File watching has been disabled. Use the "Rebuild Index" button when files change.');
      });
...
async function generateComplianceReport(reportType = 'compliance-audit', options = {}) {
...
function calculateComplianceMetrics(imsIndex, mandatoryRecords) {
  ...
  const mandatoryLinked = Object.keys(mandatoryRecords).filter(key => {
    ...
    return record.enrichedDocuments && record.enrichedDocuments.some(doc => doc.manuallyLinked);
  }).length;
  
  const complianceScore = totalDocuments > 0 ? Math.round((linkedDocuments / totalDocuments) * 100) : 0;
  const mandatoryScore = mandatoryTotal > 0 ? Math.round((mandatoryLinked / mandatoryTotal) * 100) : 0;
...
function identifyComplianceGaps(imsIndex, mandatoryRecords) {
  ...
  Object.keys(mandatoryRecords).forEach(recordType => {
    const record = mandatoryRecords[recordType];
    const hasLinkedDocs = record.enrichedDocuments && 
...
function assessComplianceRisks(gaps, companyInfo) {
  ...
  if (companyInfo.industry === 'electrical') {
    const electricalGaps = gaps.high.filter(gap => 
      gap.document && gap.document.toLowerCase().includes('electrical')).length;
    
    if (electricalGaps > 0) {
      risks.push({
        type: 'safety',
        level: 'High',
        description: 'Missing electrical safety procedures increase risk of serious injury',
        likelihood: 'Medium',
...
function generateActionPlan(gaps, risks) {
  ...
  const criticalActions = gaps.critical.map(gap => ({
    ...
    resources: gap.regulatory ? 'External consultant recommended' : 'Internal team',
  ...
  const quickWins = gaps.medium.filter(gap => gap.effort === 'Low').slice(0, 3).map(gap => ({
    priority: 3,
    description: gap.description,
    timeline: '1 week',
  ...
  return {
    ...
    quickWins: quickWins,
    totalActions: criticalActions.length + highActions.length + quickWins.length
  };
}
...
Keep recommendations practical and specific to ${aiConfig.companyInfo.industry} industry in ${aiConfig.companyInfo.jurisdiction}.`;
...
function getCriticalityLevel(documentName, categoryName) {
  const criticalKeywords = ['emergency', 'evacuation', 'lockout', 'confined space', 'electrical'];
...
function isAIGenerable(documentName) {
  const generableTypes = ['swms', 'risk assessment', 'procedure', 'policy', 'training'];
  ...
  try {
    res.render('index', { 
      ...
      lastUpdated: fs.existsSync(DB_PATH) ? 
        moment(fs.readJsonSync(DB_PATH).lastUpdated).format('YYYY-MM-DD HH:mm:ss') : 
        'Never'
    });
  } catch (err) {
...
app.get('/documents', (req, res) => {
  try {
    ...
    const visibleDocuments = documentIndex.filter(doc => !shouldHideFolder(doc.folder));
...
async function generateReportHTML(reportData, reportType) {
  const { metadata, metrics, gaps, risks, actionPlan, recommendations } = reportData;
    ...
    <div class="executive-report">
      <div class="report-header">
        ...
        <div class="report-meta">
          ...
          <p><strong>Industry:</strong> ${metadata.industry} | <strong>Jurisdiction:</strong> ${metadata.jurisdiction}</p>
        </div>
      </div>
      ...
      <div class="executive-summary">
        ...
        <div class="key-findings">
          <h3>Key Findings</h3>
          <ul>
            <li><strong>Document Compliance:</strong> ${metrics.documents.score}% (${metrics.documents.linked}/${metrics.documents.total} documents)</li>
            <li><strong>Mandatory Records:</strong> ${metrics.mandatory.score}% (${metrics.mandatory.compliant}/${metrics.mandatory.total} records)</li>
            <li><strong>Immediate Priorities:</strong> ${actionPlan.immediate.length} critical actions required</li>
          </ul>
        </div>
      </div>
      ...
      <div class="gap-analysis">
        ...
        ${gaps.critical.length > 0 ? `
          <div class="gap-section critical">
            ...
            <ul>
              ${gaps.critical.map(gap => `<li>${gap.description}</li>`).join('')}
            </ul>
          </div>
        ` : ''}
        ...
        ${gaps.high.length > 0 ? `
          <div class="gap-section high">
            ...
            <ul>
              ${gaps.high.slice(0, 10).map(gap => `<li>${gap.description}</li>`).join('')}
              ${gaps.high.length > 10 ? `<li><em>... and ${gaps.high.length - 10} more</em></li>` : ''}
            </ul>
          </div>
        ` : ''}
      </div>
  ...
  try {
    ...
    exec(`start "" "${document.path}"`, (error) => {
      if (error) {
        ...
        return res.status(500).json({ success: false, message: 'Error opening document' });
      }
...
app.get('/folders', (req, res) => {
  try {
    ...
    const visibleFolders = foldersList.filter(folder => !shouldHideFolder(folder));
  ...
  try {
    ...
    Object.keys(imsIndex).forEach(categoryName => {
      ...
      if (category.documentId) {
        category.document = documentIndex.find(doc => doc.id === category.documentId);
      } else {
      ...
      if (category.children && category.children.length > 0) {
        ...
        category.enrichedChildren = category.children.map(childName => {
          ...
          const existingChild = existingEnrichedChildren.find(child => child.name === childName);
...
app.post('/manage-hidden-folders', express.json(), (req, res) => {
  try {
    ...
    if (action === 'hide-folder') {
      ...
      hiddenConfig.hiddenFolders = hiddenConfig.hiddenFolders.filter(f => f !== folderName);
      saveHiddenFolders(hiddenConfig);
    } else if (action === 'show-path') {
      hiddenConfig.hiddenPaths = hiddenConfig.hiddenPaths.filter(p => p !== folderPath);
      saveHiddenFolders(hiddenConfig);
    }
...
app.post('/api/link-ims-document', express.json(), (req, res) => {
  try {
    const { categoryName, documentName, documentId, actualDocumentName } = req.body;
    ...
    if (!categoryName || !documentName || !documentId) {
      return res.json({
        success: false,
        message: 'Missing required parameters: categoryName, documentName, or documentId'
      });
    }
    ...
    imsIndex[categoryName].enrichedChildren = imsIndex[categoryName].enrichedChildren.map(child => {
    ...
    if (!childFound) {
      ...
      const childExists = imsIndex[categoryName].children.includes(documentName);
...
app.post('/api/replace-document/:id', upload.single('newDocument'), async (req, res) => {
  try {
    ...
    if (!originalDoc) {
      return res.json({ success: false, message: 'Original document not found' });
    }
    
    console.log('Replacing document:', originalDoc.name, 'with:', req.file.originalname);
    ...
    if (newExt === originalExt) {
      ...
      while (fs.existsSync(newPath) && newPath !== originalPath) {
        newPath = path.join(originalDir, `${originalName}_v${counter}${newExt}`);
        counter++;
      }
    }
    ...
    if (newExt !== originalExt && newPath !== originalPath && fs.existsSync(originalPath)) {
      try {
        fs.unlinkSync(originalPath);
        console.log('Removed old file with different extension');
      } catch (removeError) {
        console.warn('Warning: Could not remove old file:', removeError.message);
...
app.get('/api/mandatory-records', (req, res) => {
  try {
    ...
    Object.keys(mandatoryRecords).forEach(recordType => {
      ...
      const manualDocs = (record.enrichedDocuments || []).filter(doc => !doc.autoDetected);
      
      // Auto-detect documents
      const keywords = record.autoDetectKeywords || [];
      const autoDetectedDocs = documentIndex.filter(doc => {
...
app.post('/api/link-mandatory-record', express.json(), (req, res) => {
  try {
    ...
    const existingDocIndex = mandatoryRecords[recordType].enrichedDocuments.findIndex(doc => doc.id === documentId);
    ...
    if (existingDocIndex !== -1) {
      ...
      mandatoryRecords[recordType].enrichedDocuments[existingDocIndex].linkedAt = new Date().toISOString();
    } else {
      // Add new document
...
app.delete('/api/mandatory-record/:recordType/:documentId', (req, res) => {
  try {
    ...
    // Remove the document
    mandatoryRecords[recordType].enrichedDocuments = 
      (mandatoryRecords[recordType].enrichedDocuments || []).filter(doc => doc.id !== documentId);
...
app.post('/api/update-mandatory-record', express.json(), (req, res) => {
  try {
    const { recordType, description, keywords, requiredCount, timeframe } = req.body;
    ...
    if (requiredCount !== undefined) mandatoryRecords[recordType].requiredCount = requiredCount;
...
app.post('/api/auto-link-documents', express.json(), (req, res) => {
  try {
      ...
      if (!category.document && !category.documentId) {
        ...
        if (foundDoc) {
          if (foundDoc.isArchived) {
            skippedArchived++;
            console.log(`Skipped archived document for category: ${categoryName}`);
          } else {
            category.documentId = foundDoc.id;
            linkedCount++;
            console.log(`Linked category document: ${categoryName} -> ${foundDoc.name}`);
          }
        }
      }
        ...
        category.children.forEach(childName => {
          ...
          if (!existingChild || !existingChild.document) {
            ...
            if (foundDoc) {
              if (foundDoc.isArchived) {
                skippedArchived++;
                console.log(`Skipped archived document for child: ${childName}`);
              } else {
                ...
                if (checkRevisions) {
                  ...
                  if (revisions.length > 0) {
                    revisionsFound++;
                    console.log(`Found ${revisions.length} revisions for: ${childName}`);
                  }
                }
                ...
                linkedCount++;
                console.log(`Linked child document: ${childName} -> ${foundDoc.name}`);
              }
            }
          }
        });
    ...
    if (saveIMSIndex(imsIndex)) {
      console.log(`Auto-linking completed: ${linkedCount} linked, ${revisionsFound} revisions found, ${skippedArchived} archived skipped`);
...
app.post('/api/update-ims-category', express.json(), (req, res) => {
  try {
    ...
    console.log('Update category request:', { action, categoryName, newName, level, type, children });
    ...
    if (action === 'create') {
      ...
      if (!imsIndex[categoryName] && !newName) {
        ...
        if (children !== undefined) {
          imsIndex[targetCategoryName].children = children;
          console.log(`Updated children (legacy) for ${targetCategoryName}:`, children);
        }
      }
    }
...
app.post('/api/manage-child-document', express.json(), (req, res) => {
  try {
    ...
    console.log('Manage child document:', { categoryName, action, documentName, newDocumentName });
    ...
    switch (action) {
      case 'add':
        if (newDocumentName && !imsIndex[categoryName].children.includes(newDocumentName)) {
          imsIndex[categoryName].children.push(newDocumentName);
          console.log(`Added child "${newDocumentName}" to category "${categoryName}"`);
        }
        break;
        
      case 'remove':
        const beforeLength = imsIndex[categoryName].children.length;
        imsIndex[categoryName].children = imsIndex[categoryName].children.filter(
          child => child !== documentName
        );
        console.log(`Removed child "${documentName}" from category "${categoryName}". Count: ${beforeLength} -> ${imsIndex[categoryName].children.length}`);
        break;
      ...
      case 'rename':
        ...
        if (index > -1 && newDocumentName) {
          ...
          console.log(`Renamed child "${documentName}" to "${newDocumentName}" in category "${categoryName}"`);
        }
        break;
    ...
    if (saveIMSIndex(imsIndex)) {
      res.json({ 
        success: true, 
        message: `Child document ${action} successful`,
        children: imsIndex[categoryName].children 
      });
    } else {
      res.json({ success: false, message: 'Failed to save changes' });
    }
...
app.post('/api/ai-config', express.json(), (req, res) => {
  try {
    ...
    if (companyInfo) aiConfig.companyInfo = { ...aiConfig.companyInfo, ...companyInfo };
