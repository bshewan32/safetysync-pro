...
// Add to your existing ims-index.js file

// AI Generation Modal Handler
document.addEventListener('click', function(e) {
...
function openAIGenerationModal(categoryName, documentName) {
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="alert alert-info">
              ...
              This will create a professional safety document tailored to your company and industry.
            </div>
            ...
            <form id="aiGenerateForm">
              <div class="mb-3">
                ...
                <select class="form-select" id="documentType" required>
                  ...
                  <option value="SWMS">Safe Work Method Statement (SWMS)</option>
              ...
              <div class="mb-3">
                <label class="form-label">Activity/Topic Name</label>
                <input type="text" class="form-control" id="activityName" 
                       value="${documentName}" required>
                <small class="text-muted">Describe what this document covers</small>
              </div>
              
              <div class="mb-3">
                <label class="form-label">Specific Details</label>
                <textarea class="form-control" id="specificDetails" rows="3" 
                          placeholder="Any specific hazards, equipment, or requirements to include..."></textarea>
              </div>
              
              <div class="mb-3">
                <label class="form-label">Work Location</label>
                <input type="text" class="form-control" id="workLocation" 
                       placeholder="e.g., Construction site, Workshop, Client premises">
              </div>
              
              <div class="mb-3">
                <label class="form-label">Personnel Involved</label>
                <input type="text" class="form-control" id="personnelInvolved" 
                       placeholder="e.g., Electricians, Apprentices, Supervisors">
              </div>
              
              <!-- Cost Estimation -->
              <div class="alert alert-light" id="costEstimate">
                <small>
                  <i class="fas fa-calculator"></i> 
                  <strong>Estimated Cost:</strong> <span id="estimatedCost">$0.15 - $0.30</span>
                  <br>
                  <span class="text-muted">Based on document complexity and selected AI provider</span>
                </small>
              </div>
            </form>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
            <button type="button" class="btn btn-primary" onclick="generateAIDocument('${categoryName}', '${documentName}')">
  ...
  autoDetectDocumentType(documentName);
}

function autoDetectDocumentType(documentName) {
  const name = documentName.toLowerCase();
  let detectedType = '';
  
  if (name.includes('swms') || name.includes('safe work method')) {
    detectedType = 'SWMS';
  } else if (name.includes('risk assessment') || name.includes('hazard')) {
    detectedType = 'Risk Assessment';
  } else if (name.includes('emergency') || name.includes('evacuation')) {
    detectedType = 'Emergency Procedure';
  } else if (name.includes('training') || name.includes('competency')) {
    detectedType = 'Training Material';
  }
  
  if (detectedType) {
    document.getElementById('documentType').value = detectedType;
  }
}
...
async function generateAIDocument(categoryName, documentName) {
  ...
  const generateBtn = document.querySelector('#aiGenerateModal .btn-primary');
  generateBtn.disabled = true;
  generateBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Generating...';
  
  const requestData = {
    documentType: formData.get('documentType') || 'SWMS',
    documentName: documentName,
    customInputs: {
      activityName: formData.get('activityName'),
      specificDetails: formData.get('specificDetails'),
      workLocation: formData.get('workLocation'),
      personnelInvolved: formData.get('personnelInvolved')
    }
  };
  ...
  try {
    ...
    if (result.success) {
      alert(`✅ Document generated successfully!\n\nFile: ${result.filename}\nCost: $${result.metadata.estimatedCost.toFixed(4)}\n\nThe document has been saved to your AI Generated folder.`);
    ...
    generateBtn.innerHTML = '<i class="fas fa-robot"></i> Generate Document';
  }
}
